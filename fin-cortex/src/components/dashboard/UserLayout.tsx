...
import { Menu, Search, Moon, Sun, LogOut, Settings, User as UserIcon } from "lucide-react";
    ...
    return (
        <div className="min-h-screen bg-[var(--background-dark)] transition-all duration-500 overflow-x-hidden">
                        ...
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-8">
                                ...
                                <div className="relative group hidden sm:block">
                                    <Search className="absolute left-4 top-1/2 transform -translate-y-1/2 w-4 h-4 text-slate-500 group-focus-within:text-purple-400 transition-colors" />
                                    <input
                                        type="text"
                            ...
                            <div className="flex items-center gap-6">
                                ...
                                <DropdownMenu>
                                    ...
                                    <DropdownMenuContent align="end" className="w-64 bg-[var(--background-dark)]/90 backdrop-blur-3xl border-[var(--border-medium)] p-2">
                                        ...
                                        <DropdownMenuItem asChild className="p-3 focus:bg-white/5 cursor-pointer rounded-xl group">
                                            <Link href="/user/profile" className="flex items-center w-full">
                                                <UserIcon className="mr-3 h-4 w-4 text-slate-400 group-hover:text-white" />
                                                <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 group-hover:text-white">Profile</span>
                                            </Link>
                                        </DropdownMenuItem>
                                        <DropdownMenuItem asChild className="p-3 focus:bg-white/5 cursor-pointer rounded-xl group">
                                            <Link href="/user/profile/settings" className="flex items-center w-full">
                                                <Settings className="mr-3 h-4 w-4 text-slate-400 group-hover:text-white" />
                                                <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 group-hover:text-white">Settings</span>
                                            </Link>
                                        </DropdownMenuItem>
                                        <DropdownMenuSeparator className="bg-white/5" />
            ...
            <div className="lg:hidden relative z-10">
                <header className="bg-black/60 backdrop-blur-xl border-b border-white/5 px-6 py-4 sticky top-0 z-40">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            <Sheet open={mobileSidebarOpen} onOpenChange={setMobileSidebarOpen}>
                                <SheetTrigger asChild>
                                    <Button variant="ghost" size="icon" className="hover:bg-white/5 text-slate-400">
                                        <Menu className="w-5 h-5" />
                                    </Button>
                                </SheetTrigger>
                                <SheetContent side="left" className="w-80 p-0 bg-black border-white/10">
                                    <UserSidebar onToggle={() => setMobileSidebarOpen(false)} />
                                </SheetContent>
                            </Sheet>
